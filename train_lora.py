...
num_frames = 2 #TODO: make it work for 8
max_sequence_length, max_crops = 1500*num_frames, 12*num_frames #TODO: adjust them to fit into memory
RESIZE_DIM = None
...
# annotation_files.append('/l/users/salman.khan/workspace_pointing_lmm/datasets/annotations/annotations_ytvos_train.jsonl')
...
def collate_fn(examples):
    ...
    padded_inputs = {}
    TEXT_KEYS = ["input_ids", "loss_masks", "position_ids", "labels"]
    IMAGE_KEYS = ["images", "image_input_idx",]
    ...
    padded_inputs['response_mask'] = (padded_inputs["loss_masks"] > 0) if "loss_masks" in padded_inputs else None

    for key in padded_inputs:
        if key in ["input_ids", "labels", "image_input_idx", "position_ids", "loss_masks", "response_mask"]:
...
for name, param in model.named_parameters():
    if "lora_" in name.lower():  # Case-insensitive check
        try:
            # Extract base module path from parameter name
            base_module_path = name.rsplit(".lora_", 1)[0].rsplit(".", 1)[0]
            base_module = model.get_submodule(base_module_path)
            
            # Get device from base module's core parameters
            base_device = next(base_module.parameters()).device
            param.data = param.data.to(base_device)
            # Verify alignment
            assert param.device == base_device, (
                f"Device mismatch!\n"
                f"LoRA parameter '{name}' on {param.device}\n"
                f"Base module '{base_module_path}' on {base_device}"
            )
        except (IndexError, AttributeError) as e:
            print(f"Couldn't verify {name}: {str(e)}")
            continue
        except StopIteration:
            print(f"Base module {base_module_path} has no parameters")
            continue
