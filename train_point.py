...
from molmo_video.model3d import MolmoForCausalLM
...
output_dir = '/l/users/salman.khan/workspace_pointing_lmm/models/llm_connector_two'
...
model.model.vision_backbone.image_vit.temporal_embedding.requires_grad_(True)
# nn.init.zeros_(model.model.vision_backbone.image_vit.temporal_embedding)
nn.init.normal_(
    model.model.vision_backbone.image_vit.temporal_embedding,
    std=0.02
)
...
class CustomSFTTrainer(SFTTrainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.state.global_step % 50 == 1:
            plot_metric(self.loss_history, "Loss")
            plot_metric(self.accuracy_history, "Token Accuracy")
            plot_metric(self.point_loss_history, "Point Loss")
